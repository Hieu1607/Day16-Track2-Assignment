...
# Sau (CPU — Free Tier eligible, 1 vCPU, 1 GB RAM):
instance_type = "t3.micro"
```

> **Tại sao `t3.micro`?** Instance này nằm trong AWS Free Tier, không yêu cầu quota đặc biệt và không phát sinh chi phí EC2 trong 12 tháng đầu. Đủ để chạy LightGBM với synthetic dataset nhỏ phục vụ mục đích benchmark.
...
Môi trường đã được cài đặt tự động qua `user_data.sh` khi instance khởi động. Bạn chỉ cần SSH vào và kiểm tra:

```bash
cd ~/ml-benchmark
ls
# benchmark.py  server.py
```
...
Dataset được sinh tự động bằng `sklearn` (10,000 mẫu, 20 features) — không cần tải từ nguồn ngoài, phù hợp với giới hạn RAM của instance.
...
Kết quả sẽ được in ra terminal và lưu vào `benchmark_result.json`.

### 7.6: Kết quả Benchmark trên `t3.micro`
...
> **Ghi chú (tiếng Việt):** Với tài khoản Free Tier, EC2 `t3.micro` không tính phí (750 giờ/tháng miễn phí). Chi phí chủ yếu đến từ NAT Gateway và ALB. Hãy chụp màn hình billing ngay sau 1 giờ rồi chạy `terraform destroy` để tránh phát sinh thêm chi phí.
...
4. **Mã nguồn** thư mục `terraform/` đã chỉnh sửa (với `t3.micro`).
